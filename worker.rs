...
fn grab_cb(scope: &mut v8::HandleScope, path: &str) -> v8::Global<v8::Function> {
...
impl MainWorker {
    ...
    #[doc(hidden)]
    pub async fn run_tests(&mut self, shuffle: &Option<u64>) -> Result<(), AnyError> {
        let promise = {
            let scope = &mut self.js_runtime.handle_scope();
            let cb = self.js_run_tests_callback.open(scope);
            let this = v8::undefined(scope).into();
            let options = serde_v8::to_v8(scope, json!({ "shuffle": shuffle })).unwrap();
