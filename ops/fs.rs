...
fn op_open_sync(
    ...
    let std_file = open_options
        .open(&path)
        .map_err(|err| Error::new(err.kind(), format!("{}, open '{}'", err, path.display())))?;
...
async fn op_open_async(
    ...
    let (path, open_options) = open_helper(&mut state.borrow_mut(), &path, mode, options.as_ref())?;
...
#[op]
fn op_write_file_sync(state: &mut OpState, args: WriteFileArgs) -> Result<(), AnyError> {
...
async fn op_write_file_async(
    ...
    let write_future =
        tokio::task::spawn_blocking(move || write_file(&path, open_options, &open_args, data));
...
#[op]
fn op_fstat_sync(state: &mut OpState, rid: ResourceId) -> Result<FsStat, AnyError> {
...
#[op]
fn op_flock_sync(state: &mut OpState, rid: ResourceId, exclusive: bool) -> Result<(), AnyError> {
    use fs3::FileExt;
...
#[op]
fn op_funlock_sync(state: &mut OpState, rid: ResourceId) -> Result<(), AnyError> {
    use fs3::FileExt;
...
#[op]
async fn op_funlock_async(state: Rc<RefCell<OpState>>, rid: ResourceId) -> Result<(), AnyError> {
    use fs3::FileExt;
...
fn op_chdir(state: &mut OpState, directory: String) -> Result<(), AnyError> {
    ...
    set_current_dir(&d)
        .map_err(|err| Error::new(err.kind(), format!("{}, chdir '{}'", err, directory)))?;
    Ok(())
}
...
fn op_mkdir_sync(state: &mut OpState, args: MkdirArgs) -> Result<(), AnyError> {
    ...
    builder
        .create(&path)
        .map_err(|err| Error::new(err.kind(), format!("{}, mkdir '{}'", err, path.display())))?;
    Ok(())
}

#[op]
async fn op_mkdir_async(state: Rc<RefCell<OpState>>, args: MkdirArgs) -> Result<(), AnyError> {
    let path = Path::new(&args.path).to_path_buf();
    let mode = args.mode.unwrap_or(0o777) & 0o777;
...
#[op]
async fn op_chmod_async(state: Rc<RefCell<OpState>>, args: ChmodArgs) -> Result<(), AnyError> {
    let path = Path::new(&args.path).to_path_buf();
    let mode = args.mode & 0o777;
...
fn raw_chmod(path: &Path, _raw_mode: u32) -> Result<(), AnyError> {
    let err_mapper =
        |err: Error| Error::new(err.kind(), format!("{}, chmod '{}'", err, path.display()));
    #[cfg(unix)]
    {
        use std::os::unix::fs::PermissionsExt;
...
#[op]
async fn op_chown_async(state: Rc<RefCell<OpState>>, args: ChownArgs) -> Result<(), AnyError> {
    let path = Path::new(&args.path).to_path_buf();
...
#[op]
fn op_remove_sync(state: &mut OpState, args: RemoveArgs) -> Result<(), AnyError> {
    ...
    let err_mapper =
        |err: Error| Error::new(err.kind(), format!("{}, remove '{}'", err, path.display()));
...
#[op]
async fn op_remove_async(state: Rc<RefCell<OpState>>, args: RemoveArgs) -> Result<(), AnyError> {
    let path = PathBuf::from(&args.path);
    let recursive = args.recursive;
...
#[op]
fn op_copy_file_sync(state: &mut OpState, args: CopyFileArgs) -> Result<(), AnyError> {
...
#[op]
fn op_stat_sync(state: &mut OpState, args: StatArgs) -> Result<FsStat, AnyError> {
    let path = PathBuf::from(&args.path);
    let lstat = args.lstat;
    state.borrow_mut::<Permissions>().read.check(&path)?;
    debug!("op_stat_sync {} {}", path.display(), lstat);
    let err_mapper =
        |err: Error| Error::new(err.kind(), format!("{}, stat '{}'", err, path.display()));
...
#[op]
async fn op_stat_async(state: Rc<RefCell<OpState>>, args: StatArgs) -> Result<FsStat, AnyError> {
    let path = PathBuf::from(&args.path);
    let lstat = args.lstat;
...
#[op]
fn op_realpath_sync(state: &mut OpState, path: String) -> Result<String, AnyError> {
...
#[op]
async fn op_realpath_async(state: Rc<RefCell<OpState>>, path: String) -> Result<String, AnyError> {
    let path = PathBuf::from(&path);
...
#[op]
fn op_read_dir_sync(state: &mut OpState, path: String) -> Result<Vec<DirEntry>, AnyError> {
    let path = PathBuf::from(&path);
    ...
    let err_mapper =
        |err: Error| Error::new(err.kind(), format!("{}, readdir '{}'", err, path.display()));
...
#[op]
fn op_rename_sync(state: &mut OpState, args: RenameArgs) -> Result<(), AnyError> {
...
#[op]
async fn op_rename_async(state: Rc<RefCell<OpState>>, args: RenameArgs) -> Result<(), AnyError> {
    let oldpath = PathBuf::from(&args.oldpath);
    let newpath = PathBuf::from(&args.newpath);
    {
        let mut state = state.borrow_mut();
...
#[op]
async fn op_link_async(state: Rc<RefCell<OpState>>, args: LinkArgs) -> Result<(), AnyError> {
    let oldpath = PathBuf::from(&args.oldpath);
    let newpath = PathBuf::from(&args.newpath);
...
#[op]
fn op_symlink_sync(state: &mut OpState, args: SymlinkArgs) -> Result<(), AnyError> {
    let oldpath = PathBuf::from(&args.oldpath);
    let newpath = PathBuf::from(&args.newpath);
...
#[op]
async fn op_symlink_async(state: Rc<RefCell<OpState>>, args: SymlinkArgs) -> Result<(), AnyError> {
    let oldpath = PathBuf::from(&args.oldpath);
    let newpath = PathBuf::from(&args.newpath);
...
#[op]
fn op_read_link_sync(state: &mut OpState, path: String) -> Result<String, AnyError> {
    let path = PathBuf::from(&path);
...
#[op]
async fn op_read_link_async(state: Rc<RefCell<OpState>>, path: String) -> Result<String, AnyError> {
    let path = PathBuf::from(&path);
    {
        let mut state = state.borrow_mut();
        state.borrow_mut::<Permissions>().read.check(&path)?;
...
#[op]
fn op_ftruncate_sync(state: &mut OpState, args: FtruncateArgs) -> Result<(), AnyError> {
    let rid = args.rid;
...
#[op]
fn op_truncate_sync(state: &mut OpState, args: TruncateArgs) -> Result<(), AnyError> {
    let path = PathBuf::from(&args.path);
    let len = args.len;
...
#[op]
fn op_make_temp_dir_sync(state: &mut OpState, args: MakeTempArgs) -> Result<String, AnyError> {
...
#[op]
fn op_make_temp_file_sync(state: &mut OpState, args: MakeTempArgs) -> Result<String, AnyError> {
    let dir = args.dir.map(|s| PathBuf::from(&s));
    let prefix = args.prefix.map(String::from);
    let suffix = args.suffix.map(String::from);
...
#[op]
fn op_futime_sync(state: &mut OpState, args: FutimeArgs) -> Result<(), AnyError> {
...
#[op]
async fn op_futime_async(state: Rc<RefCell<OpState>>, args: FutimeArgs) -> Result<(), AnyError> {
...
fn op_utime_sync(state: &mut OpState, args: UtimeArgs) -> Result<(), AnyError> {
    ...
    filetime::set_file_times(&path, atime, mtime)
        .map_err(|err| Error::new(err.kind(), format!("{}, utime '{}'", err, path.display())))?;
    Ok(())
}

#[op]
async fn op_utime_async(state: Rc<RefCell<OpState>>, args: UtimeArgs) -> Result<(), AnyError> {
...
#[op]
fn op_readfile_sync(state: &mut OpState, path: String) -> Result<ZeroCopyBuf, AnyError> {
    let permissions = state.borrow_mut::<Permissions>();
...
#[op]
fn op_readfile_text_sync(state: &mut OpState, path: String) -> Result<String, AnyError> {
    let permissions = state.borrow_mut::<Permissions>();
    let path = Path::new(&path);
    permissions.read.check(path)?;
