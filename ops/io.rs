...
impl StdFileResourceInner {
    ...
    pub fn with_file<R>(&mut self, f: impl FnOnce(&mut StdFile) -> R) -> R {
        match self {
            Self::File(file) | Self::Stdin(file) | Self::Stdout(file) | Self::Stderr(file) => {
                f(file)
            }
        }
    }
    ...
    pub fn write_and_maybe_flush(&mut self, buf: &[u8]) -> Result<usize, AnyError> {
        ...
        match self {
            ...
            Self::Stdin(_) => Err(Into::<std::io::Error>::into(ErrorKind::Unsupported).into()),
            Self::Stdout(_) => {
    ...
    pub fn write_all_and_maybe_flush(&mut self, buf: &[u8]) -> Result<(), AnyError> {
...
impl StdFileResource {
    ...
    fn with_inner_and_metadata<TResult>(
        self: Rc<Self>,
        action: impl FnOnce(&mut StdFileResourceInner, &mut FileMetadata) -> Result<TResult, AnyError>,
    ...
    async fn with_inner_blocking_task<F, R: Send + 'static>(self: Rc<Self>, action: F) -> R
    where
    ...
    async fn read(self: Rc<Self>, mut buf: ZeroCopyBuf) -> Result<(usize, ZeroCopyBuf), AnyError> {
        self.with_inner_blocking_task(move |inner| -> Result<(usize, ZeroCopyBuf), AnyError> {
    ...
    async fn write(self: Rc<Self>, buf: ZeroCopyBuf) -> Result<usize, AnyError> {
        self.with_inner_blocking_task(move |inner| inner.write_and_maybe_flush(&buf))
            .await
    }

    fn with_resource<F, R>(state: &mut OpState, rid: ResourceId, f: F) -> Result<R, AnyError>
    where
...
impl Resource for StdFileResource {
    ...
    fn backing_fd(self: Rc<Self>) -> Option<std::os::unix::prelude::RawFd> {
        ...
        self.with_inner_and_metadata(move |std_file, _| Ok(std_file.with_file(|f| f.as_raw_fd())))
            .ok()
    }
}
