...
pub fn create_basic_runtime() -> tokio::runtime::Runtime {
    let (event_interval, global_queue_interval, max_io_events_per_tick) = tokio_configuration();
        ...
        rt.spawn(async move {
            ...
            tokio::spawn(async move {
                for interval in runtime_monitor.intervals() {
                    println!("{:#?}", interval);
                    // wait 500ms
                    tokio::time::sleep(std::time::Duration::from_millis(metrics_interval)).await;
                }
            });
            future.await
        })
